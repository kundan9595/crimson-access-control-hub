...
import { useStylesByBrand } from '@/hooks/masters/useStyles';
import type { Brand, Style } from '@/services/mastersService';

interface BrandStylesModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  brand: Brand | null;
}

export const BrandStylesModal: React.FC<BrandStylesModalProps> = ({
  open,
  onOpenChange,
  brand,
}) => {
  const [searchTerm, setSearchTerm] = useState('');
  const { data: styles = [], isLoading } = useStylesByBrand(brand?.id || null);
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-4xl max-h-[80vh] overflow-hidden flex flex-col">
        <DialogHeader>
          <DialogTitle>
            Styles for {brand?.name || 'Brand'}
          </DialogTitle>
          <DialogDescription>
            View and search all styles connected to this brand
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex-1 overflow-hidden flex flex-col gap-4">
          ...
          <div className="relative">
            ...
            <Input
              placeholder="Search styles by name, description, category, or status..."
              value={searchTerm}
...
export default BrandStylesModal;
