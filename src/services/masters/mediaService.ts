...
export const mediaFolderService = {
  ...
  async create(folderData: CreateMediaFolderData): Promise<MediaFolder> {
    // The path will be auto-generated by the trigger, so we don't need to include it
    const { data, error } = await supabase
      .from('media_folders')
      .insert([folderData])
      .select()
...
export const mediaItemService = {
  ...
  async incrementUsageCount(id: string): Promise<void> {
    // Since the increment_usage_count function doesn't exist, let's use a simple update
    const { error } = await supabase
      .from('media_items')
      .update({ 
