...
export class WarehouseInventoryService {
  ...
  async getInventoryByRack(rackId: string): Promise<WarehouseInventory[]> {
    try {
      ...
      // Rack locations found

      if (!locationData || locationData.length === 0) {
        // No inventory found for rack
        return [];
      }
